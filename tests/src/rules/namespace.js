...
const valid = [
  ...
  ...flatMap(getTSParsers(), (parser) => [].concat(
    ...
    // gate on the *current* parser (TS_OLD predates @typescript-eslint and still resolves these)
    parser === parsers.TS_NEW && typescriptEslintParserSatisfies('>= 8') ? [] : test({
      code: `
...
// deep dereferences //
//////////////////////
[['deep', parsers.ESPREE], ['deep-es7', parsers.BABEL_OLD]].forEach(function ([folder, parser]) { // close over params
  if (!parser) { return; }
  valid.push(
