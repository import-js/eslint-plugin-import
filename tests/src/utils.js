...
export const parsers = {
  ...
  // babel-eslint is unmaintained and is not compatible with ESLint v10+; tests that need
  // babel parsing on v10 should use parsers.BABEL_NEW (`@babel/eslint-parser`) explicitly.
  BABEL_OLD: semver.major(eslintPkg.version) < 10 ? require.resolve('babel-eslint') : false,
  BABEL_NEW: (() => {
...
export function getNonDefaultParsers() {
  // babel-eslint (BABEL_OLD) is unavailable on ESLint v10+; fall back to @babel/eslint-parser (BABEL_NEW)
