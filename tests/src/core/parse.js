...
describe('parse(path, { settings, ecmaFeatures })', function () {
  it("doesn't support JSX by default", function () {
    expect(() => parse(getFilename('jsx.js'))).to.throw(Error)
  })
  it('infers jsx from ecmaFeatures when using stock Babylon', function () {
    expect(() => parse(getFilename('jsx.js'), { ecmaFeatures: { jsx: true }}))
      .not.to.throw(Error)
  })
